...
class Profile:
    ...
    async def get_profile(self, ctx, parser):
        ...
        if member is None:
            await self.bot.say('Felhasználó nem található.')
            return
        ...
        if profile is None:
            if parser is not MyOwnProfile:
                ...
                await self.bot.say('Profil létrehozva.')
                await self.config.put(member.id, ProfileInfo())
                await ctx.invoke(self.make)
        else:
            fmt = 'Profil **{0.name}#{0.discriminator}**:\n{1}'
    ...
    async def nnid(self, ctx, *, NNID : str):
        ...
        nid = NNID.strip('"')
        if len(nid) > 16:
            await self.bot.say('NNID maximum 16 karakter lehet.')
            return
    ...
    async def squad(self, ctx, *, squad : str):
        ...
        squad = squad.strip('"')
        if len(squad) > 100:
            await self.bot.say('Maximum 100 karakter lehet..')
            return
    ...
    async def weapon(self, ctx, *, weapon : str):
        ...
        if splatoon is None:
            await self.bot.say('Splatoon parancsok kikapcsolva.')
            return
