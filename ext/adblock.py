...
class AdBlock:
    ...
    async def change_nick(self, member: discord.Member, nick_chosen: str):
        ...
        try:
            ...
            await self.bot.say("{0} nickneve sikeresen változtatva erre: '{1}'".format(member, nick_chosen))
        except discord.HTTPException:
            await self.bot.say("{0.name}-nak/nek nincs engedélye.".format(self.bot.user))
    ...
    async def censor(self):
        ...
        for server in self.bot.servers:
            for member in server.members:
                print("szerver: {0} | fh: {1.name} | role: {1.top_role} | role_id: {1.top_role.id}".format(server,
                                                                                                            member,
                                                                                                            member))
                if member.name not in whitelist:
                    for word in ads:
                        if word.lower() in str(member.nick).lower():
                            try:
                                await self.bot.change_nickname(member, '[Tiltott nick]')
                                alert = '{0.mention} nickname tiltott szavakat tartalmaz.'
                    ...
                    print("!!!!!! {0.name} ignorálva, folytatás...".format(member))
