...
class Mod:
    ...
    async def ignore(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await self.bot.say('Hibás alparancs: {0.subcommand_passed}'.format(ctx))
    ...
    async def ignore_list(self, ctx):
        ...
        if result:
            await self.bot.say('A következő csatornák ignorálva vannak:\n\n{}'.format(', '.join(result)))
        else:
            await self.bot.say('Egy csatornát sem ignorálok!')
    ...
    async def channel_cmd(self, ctx, *, channel: discord.Channel = None):
        ...
        if channel.id in ignored:
            await self.bot.say('Ezt a csatornát már ignorálod')
            return
    ...
    async def cleanup(self, ctx, search: int = 100):
        ...
        messages = ['{} üzenet el lett távolítva.'.format(deleted)]
        if deleted:
            messages.append('')
    ...
    async def kick(self, *, member: discord.Member):
        ...
        try:
            ...
            await self.bot.say('A botnak nincs permissionja kickelni.')
    ...
    async def ban(self, *, member: discord.Member):
        ...
        try:
            ...
            await self.bot.say('A botnak nincs permissionja banolni.')
        except discord.HTTPException:
            await self.bot.say('Hiba.')
        else:
            await self.bot.say('\U0001f44c')
    ...
    async def plonk(self, *, member: discord.Member):
        ...
        if member.id in plonks:
            await self.bot.say('Ez a felhasználó már nem használhatja a botot.')
            return

        plonks.append(member.id)
        await self.config.put('plonks', plonks)
        await self.bot.say('{0.name} már nem használhatja a botot..'.format(member))
    ...
    async def unplonk(self, *, member: discord.Member):
        ...
        try:
            ...
            await self.config.put('plonks', plonks)
            await self.bot.say('{0.name} már használhatja a botot.'.format(member))
    ...
    async def colour(self, ctx, colour: discord.Colour, *, role: discord.Role):
        ...
        try:
            ...
            await self.bot.say('Ehhez kell a `Manage Roles` permission.')
        else:
            await self.bot.say('\U0001f44c')
    ...
    async def remove(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await self.bot.say('IHiba: "{0.subcommand_passed}"'.format(ctx))
    ...
    async def do_removal(self, message, limit, predicate):
        ...
        spammers = Counter(m.author.display_name for m in deleted)
        messages = ['{} ki lett törölve.'.format(len(deleted))]
        if len(deleted):
            messages.append('')
            spammers = sorted(spammers.items(), key=lambda t: t[1], reverse=True)
            messages.extend(map(lambda t: '**{0[0]}**: {0[1]}'.format(t), spammers))
