    ...
    def __init__(self, argument):
        ...
        try:
            ...
            if match is None or not match.group(0):
                raise commands.BadArgument('Hiba') from e
        ...
        if self.seconds < 0:
            raise commands.BadArgument('Hiba')

        if self.seconds > 604800:  # 7 days
            raise commands.BadArgument('Már jóó sok ideje...')
...
class Meta:
    ...
    async def hello(self):
        ...
        await self.bot.say('Heló! Én egy robot vagyok.')
    ...
    async def charinfo(self, *, characters: str):
        ...
        if len(characters) > 15:
            await self.bot.say('Toúl sok karakter. ({}/15)'.format(len(characters)))
            return
    ...
    async def source(self, command: str = None):
        ...
        for cmd in code_path:
            try:
                ...
                if obj is None:
                    await self.bot.say('Parancs nem található ' + cmd)
                    return
            except AttributeError:
                await self.bot.say('{0.name} parancsnak nincsennek alparancsai'.format(obj))
                return
    ...
    async def _change_game(self, *, game_chosen: str):
        ...
        try:
            ...
            await self.bot.say("```Playing tag sikeresen meg lett változtatva erre: {0}```".format(game_chosen))
        except discord.HTTPException:
            await self.bot.say("```Playing text nem megváltoztatható valami hiba folytán.```")
    ...
    async def info(self, ctx, *, member: discord.Member = None):
        ...
        if voice is not None:
            ...
            voice = 'Nincs csatlakozva.'

        entries = [
            ('Név', member.name),
            ('DiscordTag', member.discriminator),
            ('ID', member.id),
            ('Csatlakozott', member.joined_at),
            ('FH készítve', member.created_at),
            ('Roleok', ', '.join(roles)),
            ('Szerverek', '{} közös'.format(shared)),
            ('Hang', voice),
            ('Avatár', member.avatar_url),
        ]
    ...
    async def server_info(self, ctx):
        ...
        entries = [
            ...
            ('ID', server.id),
            ('Chanelek', channels),
            ('Készítve', server.created_at),
            ('Felhasználók', member_fmt.format(len(server.members), member_by_status)),
            ('Tulajdonos', server.owner),
            ('Ikon', server.icon_url),
            ('Roleok', ', '.join(roles)),
        ]
    ...
    def get_bot_uptime(self):
        ...
        if days:
            fmt = '{d} nap, {h} óra, {m} perc, and {s} másodperc'
        else:
            fmt = '{h} óra, {m} perc, and {s} másodperc'
    ...
    async def join(self):
        """Joins a server."""
        msg = 'Ezen a linken keresztül meg tudsz hívni a saját szerveredre, hogy ott is tevénykedjek!\n\n'
    ...
    async def leave(self, ctx):
        ...
        try:
            ...
            await self.bot.say('Hiba')

    @commands.command()
    async def uptime(self):
        """Tells you how long the bot has been up for."""
        await self.bot.say('A bot ennyi ideje aktív: **{}**'.format(self.get_bot_uptime()))
    ...
    async def about(self):
        ...
        result = ['**Rólam:**']
        result.append('- Készítőm: Steven')
        result.append('- Library: discord.py (Python)')
        result.append('- ENnyi ideje vagyok aktív: {}'.format(self.get_bot_uptime()))
        result.append('- Szerverek: {}'.format(len(self.bot.servers)))
        result.append('- Parancsok: {}'.format(sum(self.bot.commands_used.values())))
        ...
        result.append('- Össz. felhasználók: {} ({} online)'.format(total_members, total_online))
        result.append('- Felhasználók: {} ({} online)'.format(len(unique_members), unique_online))
        result.append('- {} text csatornák, {} voice channels'.format(text, voice))
        result.append('')
        await self.bot.say('\n'.join(result))
    ...
    async def commandstats(self):
        msg = 'Az indítás óa, {0} parancs volt lefuttatva.\n{1}'
